...
// see https://stackoverflow.com/questions/11381673/detecting-a-mobile-browser
  ...
  if (isMobile) {
    ...
    isPhone = (smallerEdgeSize <= 480) && (largerEdgeSize <= 896)
  }
...
/**** determine presence and accuracy of primary pointing device ****/
  ...
  type DevicePointingAccuracy = 'none' | 'fine' | 'coarse'

  let PointingAccuracy:DevicePointingAccuracy       // may change while running!

  function updatePointingAccuracy ():void {
    let updatedPointingAccuracy:DevicePointingAccuracy = 'fine'
    switch (true) {
    ...
    PointingAccuracy = updatedPointingAccuracy
...
/**** onDevicePointingAccuracyChanged ****/

  function onDevicePointingAccuracyChanged (Handler:Function):void {
    registerHandler(Handler,false)
  }

/**** onceDevicePointingAccuracyChanged ****/

  function onceDevicePointingAccuracyChanged (Handler:Function):void {
    registerHandler(Handler,true)
  }

/**** offDevicePointingAccuracyChanged ****/

  function offDevicePointingAccuracyChanged (Handler:Function):void {
    unregisterHandler(Handler)
  }

/**** observePointingAccuracy (e.g., on "convertibles") ****/

  let AccuracyPoller = undefined

  function observePointingAccuracy ():void {
    AccuracyPoller = setInterval(() => {
      let oldPointingAccuracy = PointingAccuracy
        updatePointingAccuracy()
      if (PointingAccuracy !== oldPointingAccuracy) {
...
/**** rewrite media query rules for legacy browsers on touch devices ****/
  ...
  /**** rewrite media query rules for legacy browsers ****/
      ...
      for (let j = 0, k = Rules.length; j < k; j++) {
        ...
        if (Rule.type === CSSRule.MEDIA_RULE) {
          if (ListHasElementMatching(Rule.media, /handheld/i)) {
            let Media = Rule.media                        // reference, not copy
  ...
  export default {
    get DeviceIsMobile () { return isMobile },
    get DeviceIsPhone ()  { return isPhone },
    get DeviceIsTablet () { return isMobile && ! isPhone },

    get DeviceIsLegacyTouchDevice () { return AppRunsOnLegacyTouchDevice },

    get DevicePointingAccuracy () { return PointingAccuracy },
